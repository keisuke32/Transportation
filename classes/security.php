...
class Security
{
	...
	public static function getUserGroups()
	{
		...
		$sql = "select ". $grConnection->addFieldWrappers( "Label" )
			." from ". $grConnection->addTableWrappers( "targettransportation_uggroups" ) . " WHERE " . $grConnection->addFieldWrappers( "GroupID" )
	...
	public static function getUgMembersDatasource() {
		...
		global $cman;
		return getTableDataSource( "targettransportation_ugmembers", $cman->getUserGroupsConnId() );
	}
	...
	public static function getUgGroupsDatasource() {
		...
		global $cman;
		return getTableDataSource( "targettransportation_uggroups", $cman->getUserGroupsConnId() );
	}
	...
	public static function getUgRightsDatasource() {
		...
		global $cman;
		return getTableDataSource( "targettransportation_ugrights", $cman->getUserGroupsConnId() );
	}
	...
	public static function verifyPassword( $password, $storedPassword ) {
		...
		if( $globalSettings["bEncryptPasswords"] ) {
			if( $globalSettings["nEncryptPasswordMethod"] == 0 )
				...
				return Security::hashPassword( $password ) === $storedPassword;
			}
		}
