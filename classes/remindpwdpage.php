...
class RemindPasswordPage extends RunnerPage
{
	...
	{		
		...
		if( $this->remindMethod() == RPM_RESET )
		{
			...
			$this->connection->exec("update ".$this->connection->addTableWrappers("tbl_users")." set "
		...
		else
		{
				if( !$this->cipherer->isFieldEncrypted( $this->passwordFiled ) )
			{
				...
				$this->connection->exec("update ".$this->connection->addTableWrappers("tbl_users")
 ...
 //RPM_RESET
	...
	protected function getUserSearchingSQLData()
	{
		...
		{
			$fullcUserNameField = $this->connection->addTableWrappers( "tbl_users" ).".".$this->connection->addFieldWrappers( $this->usernameFiled );
			$fullcEmailField = $this->connection->addTableWrappers( "tbl_users" ).".".$this->connection->addFieldWrappers( $this->emailField );
			$fullcPasswordField = $this->connection->addTableWrappers( "tbl_users" ).".".$this->connection->addFieldWrappers( $this->passwordFiled );
		}
		...
		$strSQLData["sql"] = $selectClause." from ".$this->connection->addTableWrappers("tbl_users")." where ".$sWhere;
	...
	function assignBodyEnd($params = "") 
	{
		...
		echo "<script language=\"JavaScript\" src=\""
			.GetRootPathForResources("include/runnerJS/RunnerAll.js?36520")."\"></script>\r\n";
