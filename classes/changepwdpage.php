...
class ChangePasswordPage extends RunnerPage
{
	...
	protected function getUpdateCommand( $newpass ) {
		...
		if( GetGlobalData( "userRequireActivation" ) && $this->token ) {
			$values[ "reset_token" ] = "";
	 ...
	 * @param String dbOldPass
	 * @param Boolean bcrypted
	 * @return Boolean
	 */
	protected function checkOldPasswordValue( $oldPass, $dbOldPass, $bcrypted ) {
		if( !strlen( $dbOldPass ) )
			return false;
		...
		if( $bcrypted )
			return passwordVerify( $oldPass, $dbOldPass );

		return $oldPass == $dbOldPass;
	}
	...
	protected function changePassword() {
		...
		if( !$this->checkOldPasswordValue( $oldPass, $dbOldPass, $bcrypted ) ) {
