...
{
	$value = "";
				if($table=="forms" && $field=="formfile")
	{
		;
		return $value;
	}
				if($table=="forms" && $field=="pdffile")
	{
		;
		return $value;
	}
...
function GetDefaultValue($field, $ptype, $table="")
{
	...
	if(!$table)
		$table=$strTableName;
				if($table=="tbl_order_status" && $field=="StatusDate")
	{
		return now();;
	}
				if($table=="tbl_order_status" && $field=="Private")
	{
		return 0;
	}
				if($table=="tbl_orders" && $field=="companyID")
	{
		return $_SESSION["CompanyID"];
	}
				if($table=="tbl_orders" && $field=="Inbound")
	{
		return 0;
	}
				if($table=="tbl_company" && $field=="Insurance")
	{
		return 0;
	}
				if($table=="tbl_rfqs" && $field=="RFQStatusID")
	{
		return 1;
	}
	return "";
